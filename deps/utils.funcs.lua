...
function scope.match(val, arg, ...)
  ...
  return select("#", ...) > 0 and scope.call(val, scope.match, ...) or false
end
...
function scope.enqueue(key, fn)
  if fn or not match(fn, unpack(scope.read(scope, key) or scope.empty)) then
...
function scope.createFrame(kind, name, parent, template, mixin)
