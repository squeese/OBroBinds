...
scope.enqueue("ADDON_ROOT_SHOW", setmetatable({
  ...
  scope.STACK.init, scope.createRootPanels,
  scope.STACK.init, scope.createScrollbar,
  scope.STACK.both, function(next, ...)
    if next.key == "ADDON_ROOT_SHOW" then
      scope.dbWrite('GUI', 'open', true)
      scope.panel.Pages[PanelTemplates_GetSelectedTab(scope.panel)]:Show()
    else
      scope.dbWrite('GUI', 'open', nil)
      scope.panel.Pages[PanelTemplates_GetSelectedTab(scope.panel)]:Hide()
    end
    return next(...)
  end,
}, scope.STACK))

scope.enqueue("ADDON_PAGE_KEYBOARD_SHOW", setmetatable({
  scope.STACK.fold, "ADDON_PAGE_KEYBOARD_HIDE",
  ...
  scope.STACK.call, function(e, ...)
    scope.dbWrite("GUI", "page", scope.tabKeyboard:GetID())
    return e(...)
  end,
